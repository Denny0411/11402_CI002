...
> 你問到：「這樣排序後，該如何照原本的輸入排序輸出？」
> 好消息是：**你其實不需要還原回原本的輸入順序！**
> UVa 410 這類題目通常具備「Special Judge（特判機制）」。這意味著只要你的答案滿足以下條件：
> 
> 1. 每個 Chamber 最多分配 2 個樣本 。
> 2. 計算出來的 IMBALANCE 是所有可能組合中最小的 。
> 
> 就算你輸出的 Chamber 順序（例如 `0:`、`1:`）或是 Chamber 內部的樣本順序與範例輸出**不完全相同**，系統依然會判定你的答案是正確的（Accepted）。所以，你可以按照排序後配對的順序直接輸出，不需要費心記錄原本的輸入位置。
>
> written by gemini
